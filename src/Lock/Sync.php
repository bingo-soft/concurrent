...
abstract class Sync extends AbstractQueuedSynchronizer
{
     ...
     */
    abstract public function lock(ThreadInterface $thread): void;

    /**
     * Performs non-fair tryLock.  tryAcquire is implemented in
     * subclasses, but both need nonfair try for trylock method.
     */
    public function nonfairTryAcquire(ThreadInterface $current, int $acquires): bool
    {
        $c = $this->getState();
        if ($c == 0) {
    ...
    public function tryRelease(ThreadInterface $current, int $releases): bool
    {
